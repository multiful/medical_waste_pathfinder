...
# -------------------------------
with tab2:
    st.markdown("### 전국 시설유형별 병원당 의료폐기물 (선택 지표 기준)")

    if TARGET_USED in df.columns:
        total_waste_all = df[TARGET_USED].sum()
                    ...
                    facility_totals.append(
                        {
                            "facility": col,
                            "병원수": tot,
                            "waste_per_facility": total_waste_all / tot,
                        }
                    )
            ...
            with c1:
                bar = (
                    ...
                    .encode(
                        ...
                        y=alt.Y("waste_per_facility:Q", title="병원 1기관당 의료폐기물(선택 지표)"),
        ...
        st.warning("선택된 의료폐기물 컬럼이 없어 시설유형 분석을 수행할 수 없습니다.")
...
with tab3:
    ...
    if TARGET_USED not in df.columns:
        ...
        with st.expander("표 보기 (상관계수)"):
            st.dataframe(
                corr_df.drop(columns="abs_r").sort_values("Pearson r", ascending=False),
...
""",
        ...
        # wide → long (Altair transform_fold 대신 pandas melt 사용)
